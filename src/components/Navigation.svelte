...
<nav class="px-4 md:px-20 lg:px-48 border-b border-gray-200">
      <div class="flex items-center justify-between w-full">
        <a href="/" class="text-gray-800 px-4 min-w-24 text-xl z-20" on:click={() => isMenuOpen=false}>
            <h1>Crafty</h1>
        </a>
          ...
          <div class="absolute w-5 transform -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
            <span
              class={`absolute h-0.5 w-5 transform transition duration-300 ease-in-out ${
                isMenuOpen ? "rotate-45  delay-200 bg-gray-50" : "-translate-y-1.5 bg-gray-600"
              }`}
            ></span>
            <span
            ...
            <span
              class={`absolute h-0.5 w-5 transform transition duration-300 ease-in-out ${
                isMenuOpen ? "-rotate-45 bg-gray-50 delay-200" : "translate-y-1.5 bg-gray-600"
              }`}
            ></span>
          </div>
        </button>
    ...
    <nav
        class={`fixed flex top-0 left-0 w-full z-40 h-screen bg-gray-600 text-primary bg-opacity-100 transform delay-100 transition-all duration-500 ${
          isMenuOpen
