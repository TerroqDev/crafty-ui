<script>
	...
	onMount(async () => {
		if (data) {
			const response = await fetch('http://crafty-service:4000/test');
			const responseData = await response.json();
			if (responseData) {
				// @ts-ignore
				const fetchUser = await fetch(`http://crafty-service:4000/users/${responseData.email}`);
				if (fetchUser.status === 404) {
					const createUser = await fetch(`http://crafty-service:4000/users/`, {
						method: 'POST',
						headers: {
