  ...
  onMount(async () => {
    if(data){
        ...
        if(responseData){
            ...
            // @ts-ignore
            user.set({email: responseData.email, picture: responseData.picture, firstName: databaseUser.firstName, lastName: databaseUser.lastName, instagram: databaseUser.instagram, whatsapp: databaseUser.whatsapp, nickname: databaseUser.nickname});
        }else{
            console.log("no data")
        }
    }
