...
{#if fullSizeImage}
   <div on:click={() => fullSizeImage = false} class="fixed overflow-hidden h-lvh inset-0 z-50 bg-black bg-opacity-80 flex items-center justify-center">
 <img
	...
	<div class="grid grid-cols-1 md:grid-cols-2 md:gap-8 pt-8">
		<div bind:this={galleryEl} class="md:p-8">
			<div class="relative w-full aspect-[16/9]">
				<img class="rounded-lg absolute inset-0 w-full h-full object-cover cursor-pointer" on:click={() => fullSizeImage = true} src={data.product.images[imageCounter].image_url} />
				<button
			...
			<div class="flex gap-2 overflow-scroll">
				{#each data.product.images as image, i}
					...
					>
						<img class="rounded-lg absolute inset-0 w-full h-full object-cover" src={image.image_url} alt="Product image" />
					</div>
				{/each}
			</div>
		</div>
